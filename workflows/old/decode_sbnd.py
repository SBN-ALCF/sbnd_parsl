...
# generic stage type, no special rules
POT = StageType("pot")
...
def runfunc(self, fcl, inputs, run_dir, executor, last_file=None, nevts=-1, nskip=0):
    ...
    output_dir = executor.output_dir / self.stage_type.name / run_number_str / f'{executor.run_counter // 10:06d}'
    ...
    cmd = f'mkdir -p {run_dir} && cd {run_dir} && lar -c {fcl} {input_file_arg_str} {output_file_arg_str} --nevts={nevts} --nskip={nskip} --tmpdir={run_dir}'
...
class DecoderExecutor(WorkflowExecutor):
    ...
    def setup_single_workflow(self, iteration: int, rawdata_files: List[pathlib.Path], last_file=None):
        ...
        for i, file in enumerate(rawdata_files):
            ...
            s4 = Stage(POT)
            s2.add_parents(s4, workflow.default_fcls)

            s5 = Stage(DECODE)
            s5.add_input_file(str(file))
            s5.combine = True
            s4.add_parents(s5, workflow.default_fcls)
