...
class DecoderExecutor(WorkflowExecutor):
    ...
    def setup_single_workflow(self, iteration: int, rawdata_files: List[pathlib.Path]):
        ...
        # test splitting input files into a few events each
        ...
        for i, file in enumerate(rawdata_files):
            ...
            # reco1_stages = [
            #     Stage(StageType.RECO1, runfunc=reco1_runfuncs[j]) \
            #     for j in range(10)
            # ]
            # s2.add_parents(reco1_stages, workflow.default_fcls)
            ...
            if 'decode' in self.fcls:
                ...
                # for j, s3 in enumerate(reco1_stages):
                #     s3.add_input_file(str(file))
                #     s3.run_dir = s2.run_dir / f'{j:03d}'
